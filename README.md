# Sistem Pelaporan & Tindak Lanjut Keluhan Pelanggan

Proyek Akhir Mata Kuliah **Pemrograman Berbasis Web (SIF-1402)**  
Universitas Pignatelli Triputra - Program Studi Sistem Informasi (P1)

## 👤 Ide ntitas Mahasiswa
- **Nama**Celsiana Tantri
- **NIM:** 43026028
- **Angkatan: 2024a
- **Link Video Demonstrasi:

## 📝 Deskripsi Proyek
Sistem informasi berbasis web untuk mengelola pelaporan keluhan pelanggan secara digital. Sistem ini memungkinkan pelanggan untuk melaporkan masalah, melacak status tiket secara real-time, serta memberikan rating kepuasan. Di sisi admin, sistem menyediakan dashboard analitik untuk monitoring SLA (Service Level Agreement) dan pengambilan keputusan berbasis data.

## 🛠️ Tech Stack
- **Frontend:** HTML5, CSS3, JavaScript (Vanilla ES6+), Chart.js (untuk dashboard)
- **Backend:** Node.js, Express.js
- **Database:** MySQL
- **ORM:** Prisma ORM
- **Autentikasi:** JWT (JSON Web Token) & bcry*  pt
- **Tools:** Postman (testing API), Git

## 🚀 Fitur Utama
1. **Autentikasi:** Register, Login, Logout (Role: Customer & Admin).
2. **CRUD Keluhan:**
   - Customer: Buat laporan, dan tracking status.
   - Admin: Validasi, update status, tambah catatan tindak lanjut.
3. **Dashboard Admin:** Grafik statistik keluhan, monitoring SLA, tabel data interaktif.
4. **Audit Log:** Mencatat setiap perubahan status tiket.
5. **Export Data:** Export laporan ke format CSV/PDF (Nilai Tambah).

## 📂 Struktur Folder
